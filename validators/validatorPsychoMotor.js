const service = require('../services/servicePsychoMotor');
...
function getPsychoMotor(filterPsychoMotor){
  return new Promise((resolve) => {
    resolve(service.list(filterPsychoMotor));
  });
};
...
function updatePsychoMotor(id, fallsEasily, walkWithDifficulty, feetRequiresSupport, stumblesOnObjects, cannotSwitchActionsEasily, useAllFingers, doesNotControlItsStrokes, doesNotTakeObjectsProperly, observations){
  return new Promise ((resolve, reject) => {
    ...
    const result = service.update(id, fallsEasily, walkWithDifficulty, feetRequiresSupport, stumblesOnObjects, cannotSwitchActionsEasily, useAllFingers, doesNotControlItsStrokes, doesNotTakeObjectsProperly, observations);
    resolve(result);
  });
};
